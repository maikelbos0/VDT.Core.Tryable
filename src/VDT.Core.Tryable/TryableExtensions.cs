...
public static class TryableExtensions {
    ...
    public static Tryable<TValue> Catch<TException, TValue>(this Tryable<TValue> tryable, Func<TException, TValue> handler, Func<TException, bool> filter) where TException : Exception {
        tryable.ErrorHandlers.Add(new ErrorHandler<TException, TValue>(handler, filter));
        return tryable;
    }
